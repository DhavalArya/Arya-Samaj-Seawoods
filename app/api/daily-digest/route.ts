...
function logEnvSanity() {
  const pj = process.env.FB_PROJECT_ID;
  const ce = process.env.FB_CLIENT_EMAIL;
  const pk = process.env.FB_PRIVATE_KEY;
//   console.log("[env] FB_PROJECT_ID:", pj);
//   console.log("[env] FB_CLIENT_EMAIL:", ce);
//   console.log("[env] FB_PRIVATE_KEY length:", pk ? pk.length : "MISSING");
  if (pk) {
    const hasEscaped = pk.includes("\\n");
    // console.log("[env] FB_PRIVATE_KEY hasEscapedNewlines:", hasEscaped);
    // console.log("[env] FB_PRIVATE_KEY startsWith BEGIN:", pk.includes("BEGIN PRIVATE KEY"));
  }
}
...
function getServiceAccount() {
  ...
  // Accept either escaped \n or base64 (see note below)
  ...
  if (!key.includes("BEGIN PRIVATE KEY")) {
    // console.error("FB_PRIVATE_KEY looks malformed (no BEGIN PRIVATE KEY)");
    throw new Error("FB_PRIVATE_KEY malformed");
  }
...
// ---- Firebase Admin init ----
if (!getApps().length) {
    logEnvSanity(); // TEMP: see what’s actually loaded
    const key = (process.env.FB_PRIVATE_KEY || "").includes("\\n")
...
function fmtIN(d?: Date): string { return d ? new Intl.DateTimeFormat("en-IN").format(d) : ""; }
function isBirthdayToday(dob: any, M: string, D: string): boolean {
  const dt = extractDate(dob);
  if (!dt) return false;
  return String(dt.getMonth()+1).padStart(2,"0") === M && String(dt.getDate()).padStart(2,"0") === D;
}
function htmlTable(title: string, rows: string[][], cols: string[]) {
  if (!rows.length) return `<h3>${title}</h3><p>None</p>`;
  const head = cols.map(c => `<th style="text-align:left;padding:6px 10px;">${c}</th>`).join("");
  const body = rows.map(r => `<tr>${r.map(v => `<td style="padding:6px 10px;">${v ?? ""}</td>`).join("")}</tr>`).join("");
  return `<h3>${title}</h3>
...
export async function GET(req: NextRequest) {
  // Allow Vercel cron in prod; allow manual test in dev
...
//   }
  ...
  const html = `
    <div style="font-family:system-ui,Segoe UI,Arial,sans-serif">
      ...
      ${htmlTable("🎂 Birthdays Today", birthdays, ["Name","Mobile","Email","DOB"])}
      <br/>
      ${htmlTable("🗓️ Memberships Ending Today", expiring, ["Name","Mobile","Email","Valid Till","Payment Ref"])}
